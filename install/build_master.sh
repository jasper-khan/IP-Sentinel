...
for mod in "${MODULES[@]}"; do
    if ! curl -fsSL --connect-timeout 10 --retry 3 "${REPO_RAW_URL}/install/${mod}?t=$(date +%s)" -o "${SECURE_TMP}/${mod}" || [ ! -s "${SECURE_TMP}/${mod}" ]; then
        echo -e "\033[31m❌ 致命错误：中枢依赖模块 [${mod}] 装载失败！\033[0m"
        exit 1
    fi

    # [V3 供应链门禁] 模块哈希须与 MANIFEST.sha256 一致
    MOD_EXPECTED=$(awk -v f="install/${mod}" '$2 == f {print $1}' "${SECURE_TMP}/MANIFEST.sha256" 2>/dev/null)
    ...
    if ! bash -n "${SECURE_TMP}/${mod}"; then
        echo -e "\033[31m❌ 依赖模块 [${mod}] 语法校验失败，拒绝执行。\033[0m"
        exit 1
    fi
...
if ! do_fetch_master_version; then
    exit 1
fi
...
if ! do_master_stage_payloads; then
    exit 1
fi
if ! do_master_deploy_core; then
    do_master_rollback
    exit 1
fi
if ! do_engine_setup; then
    do_master_rollback
    exit 1
fi
if ! do_master_finalize_version; then
    do_master_rollback
    exit 1
fi
