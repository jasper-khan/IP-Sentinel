...
## [v5.6.36-fork] - 2026-09-13

### Fixes

- 失败启动不占用调度槽位，避免阻塞后续正常节点。
- 会话计数排除 timeout 包装进程，恢复空闲槽位补位。
- 暂停自动养护仍支持手动触发；节点忙碌或启动失败时保留请求。
- 5 项调度器离线回归测试通过；本次仅更新 Master，Agent 保持 v5.6.34，未执行生产 OTA。
...
- **普通 OTA 不再重铸 TLS 证书** — 原安装器每次升级都会先删除证书，随后整组替换 `core/` 又再次丢失本地证书，导致所有节点在 OTA 后更换指纹并触发“PSK 挑战验证为合法轮换”的 Telegram 消息。现改为在原子换血前验证现有证书与私钥配对，验证通过则原样继承；缺失、损坏或旧版证书仍由守护进程安全重建，PSK/MITM 防线不变。
- **Agent 版本探针绕过 GitHub Raw 缓存** — `version.txt` 请求加入时间戳，避免刚发版后个别节点读到旧 `AGENT_VERSION` 而错误跳过；跳过回执同时显示本地与远端版本，便于定位。
...
- **安装链补齐两处锁定哈希门禁** — `master/tg_master.sh` 与 4 个引擎文件都在 MANIFEST 锁定范围内，但安装时**都不做校验**。同一条链上的 `install_master.sh` / `build_master.sh` 早已是哈希熔断，唯独这两处漏着：
  - **`install/master_setup.sh` 部署中枢核心时只查非空** — 实测把产物换成后门版本，安装照单全收并覆盖正在运行的司令部（`tg_master.sh` 是 root 常驻、持有 TG 令牌与 PSK 的最高权限产物）。现补上与 `build_master.sh` 同构的校验：清单缺失、条目缺失、哈希不符一律熔断，失败路径不碰正在运行的中枢（沿用防砖语义）
  - **`install/engine_setup.sh` 拉 4 个引擎文件时直接 `-o` 覆盖目标路径且无任何校验**，拉取失败只打一行黄字就继续装（传输中断会把半截文件留在原地）。现改为**整组原子落位**：先全部下载到暂存目录并逐个对 MANIFEST 校验，全数通过才统一 `mv` 进 `engine/`；任一失败即熔断并清空暂存 —— 杜绝"新调度器 + 旧浏览器引擎"的混版状态
  - 重试沿用本文件时区表那条实测经验：退出条件是"哈希对上了"而非"拿到了文件"（curl 的 `--retry` 不重试 404，而 raw.githubusercontent 的间歇 404 是瞬时故障）
  - 发布形态：仅主控通道 `v5.6.32-fork`（AGENT_VERSION 未变，不打 `-agent` tag）

## [v5.6.31-fork] - 2026-09-13

### 🐛 Fixes

- **中枢 OTA 熔断时临时文件收不掉** — 下载失败分支靠 `|| VAR=""` 置空变量来标记失败，而下方 `rm -f "$VAR"` 对空串是空操作（`rm -f ""` 返回 0 且不删任何东西）。两个临时文件由 `mktemp` 在下载**之前**建好，因此**每次熔断必然留下一个脚本 + 一个清单**（实测 2 个），只能靠下一轮 OTA 开头的历史回收兜底。修复：变量保留路径，判空改看文件本身（`[ ! -s "$MANIFEST_TMP" ]`），熔断分支的 `rm` 恢复生效；成功路径行为不变（交接脚本仍保留给 `systemd-run`/`nohup` 接手执行）
  - 顺带消除同源的阻塞隐患：变量置空后，下一行 `awk '$2 == "..." {print $1}' "$MANIFEST_TMP"` 会拿到**空文件名**，gawk 此时回落读 **stdin**——stdin 一旦不关闭即永久阻塞。systemd/cron 下 stdin 为 `/dev/null` 故未显现，但手工从终端运行 `bash tg_master.sh` 会让整个 OTA 处理挂死。实测：修复前 6s 超时(退出码 124)，修复后正常走完
  - 发布形态：仅主控通道 `v5.6.31-fork`（AGENT_VERSION 未变，按发布纪律不打 `-agent` tag）
