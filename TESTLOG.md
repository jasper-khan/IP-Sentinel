...
## v5.6.27 每日简报重构 (2026-09-11, 002)

### 用户反馈
- "现在总的汇报太乱了，难看"。定位到 tg_digest.sh 四处缺陷

### 缺陷与修复
1. **数字矛盾** (coolduck "24h 12 次 (✅0🔴13)"): 次数按"会话结束"行、达成率按"区域自检"行两个独立计数, 24h 窗口边界错位 → 会话数以"区域自检"行为准 (每会话恰好一条), 次数==判定分母
2. **DRIFT 压成 🔴**: 原 `else badv++` 一切非 OK 全算红 → 分级 (✅🟠🟡🔴⚪) 只列非零, 酷鸭 13 次 DRIFT 从全红变 🟠14
3. **已接受状态天天报警**: 非 OK 即 FLAG++ → 报警只认 SINICIZED, DRIFT 恒常态不占头条
4. **OK 行信息过载**: 三核全目标区无信息量 → OK 节点压缩成 `🟢 目标达成 · 时间`, 异常节点才展开三核+落地域名

### 002 验证 (DIGEST_DRYRUN=1, git blob 字节一致)
- coolduck: `24h 14 次 · 0% (✅0 🟠14)` — 14==14 自洽, 🟠 而非 🔴
- cloudnium02: `93% (✅13 🟡1)` — 原 🔴1 实为 WATCH
- OK 节点一行; 头条无 ⚠️ (酷鸭 DRIFT 不再报警)
- 顺带观察到 45min 间隔生效: 24h 纠偏 75→82 次

## v5.6.29 Agent 日报重构 + OTA tag 通道拆分 (2026-09-11, 002)

### 用户反馈
- "每个机器的每日简报也太乱了"。定位到 core/tg_report.sh (agent 侧日报)

### 缺陷与修复
1. **版本显示误导**: 原"当前运行版本: v5.6.11 (✅已是最新)"——agent 与 master 版本独立, 中枢已升 5.6.x 后这条极具困惑 → 版本分轨: `⚙️ 中枢 vX · 代理 vY ✅与仓库一致/✨代理有新版可 OTA` (一次拉 version.txt 同时取两行)
2. **布局重叠**: "每日简报"里"节点引擎状态"+"系统引擎状态"两段状态重叠 → 合并成单卡片, 统一分隔线
3. **时间啰嗦**: 守护"自 Wed 2026-09-09 02:41:34"→"运行自 09-09 02:41"; 巡检同精到分
4. **促销尾缀**: GitHub 求星 footer 删除
5. 附带 `TG_REPORT_DRYRUN=1` 预览模式 (与 Master 简报 DIGEST_DRYRUN 对齐)

### 002 验证 (TG_REPORT_DRYRUN=1, git blob 字节一致)
```
📊 *节点日报 · cloudnium02*  (🇺🇸 US)
📍 出口 IP: `192.255.172.110` · HostPapa 🏠
────────────────────
🩺 守护进程: 🟢 active · 运行自 2026-09-09 02:41
🔌 指令端口: `23406`
🧹 最近巡检: 2026-09-10 02:41
────────────────────
⚙️ 中枢 v5.6.27 · 代理 v5.6.11 ✅ 与仓库一致
⏱️ 2026-09-11 00:57 UTC
```
(显示 5.6.27/5.6.11 因尚未 push; push 后仓库 5.6.28/5.6.12)

### 发布形态 (命名空间撞车 + 拆分)
- 实测 bump agent 到 5.6.12 即撞已存在的 `v5.6.12-fork` (历史 master 5.6.12 tag): v5.6.0~v5.6.28 全被历史 master 占用, 任何 5.6.x agent 版本都无法发布
- 修复: agent 通道改为 `v{VER}-agent` (agent_daemon.sh 内嵌 OTA 脚本 TAG_URL + 注释); master 仍 `v{VER}-fork`
- 本次单 tag `v5.6.29-fork` 双通道桥接: MASTER 5.6.27→5.6.29 + AGENT 5.6.11→5.6.29。旧 agent 经旧 `-fork` 方案拉取该 tag, install.sh 装上带新 `-agent` 方案的 agent_daemon, 此后两通道独立命名
